...
$flashSaleOptions = $fetchAllRows(
    $db,
    'SELECT p.id, p.brand, p.model, p.stock, p.selling_price, b.name AS branch_name
     FROM phones p
...
$flashSales = $fetchAllRows(
    $db,
    'SELECT fs.*, p.brand, p.model, p.selling_price AS regular_price, p.stock, b.name AS branch_name,
            CASE
...
<style>
  :root{--bg:#f5f7fb;--card:#fff;--text:#111827;--muted:#6b7280;--border:#dbe3ef;--accent:#f97316;--accent-2:#fb923c;--good:#0f9d58;--warn:#d97706;--bad:#dc2626}
  *{box-sizing:border-box} body{margin:0;font-family:Arial,sans-serif;background:linear-gradient(180deg,#fff7ed 0,#f5f7fb 28%);color:var(--text)}
  ...
  .eyebrow{font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:var(--accent);font-weight:700}.title{font-size:34px;font-weight:800;margin:6px 0}.sub{color:var(--muted);max-width:720px;line-height:1.6}
  ...
  .grid{display:grid;grid-template-columns:380px 1fr;gap:18px}.card{background:var(--card);border:1px solid var(--border);border-radius:20px;padding:18px}.card h2{margin:0 0 14px;font-size:18px}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
          ...
          <?php foreach ($flashSales as $flashSale): ?>
            <tr>
              <td>
                ...
                <div class="meta"><?= e(str_replace('RF Chein - ', '', $flashSale['branch_name'] ?? '—')) ?></div>
              </td>
              <td>
